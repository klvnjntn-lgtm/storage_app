...
export type InvoicePrintView = {
  ...
  total: number;
  amountPaid: number;
  creditedAmount: number; // NEW — sum of return-driven AR reversals; see backend Invoice.creditedAmount
  ...
  // NEW — individual payment rows, so the detail page can list and void them.
  payments: {
    id: string;
    amount: number;
    method: string;
