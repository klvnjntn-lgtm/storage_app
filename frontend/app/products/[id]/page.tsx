...
type EventRow = {
  ...
  createdAt: string;
  userId?: string | null;
  user?: { email: string } | null;   // if you include the relation
    ...
    const load = async () => {
      try {
        const [productRes, stockRes, eventsRes] = await Promise.all([
          apiFetch(`http://localhost:3000/products/${id}`),
          apiFetch(`http://localhost:3000/stock/${id}`),
          apiFetch(`http://localhost:3000/products/${id}/events`),
        ]);
      ...
      <div className="flex flex-wrap gap-3 items-end">
        ...
        <button
          ...
          onClick={async () => {
            ...
            const res = await fetch('http://localhost:3000/stock/adjust', {
              method: 'POST',
            ...
            const eventsRes = await fetch(`http://localhost:3000/products/${product.id}/events`);
