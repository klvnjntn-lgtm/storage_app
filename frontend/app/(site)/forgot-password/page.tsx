...
export default function ForgotPasswordPage() {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (!res.ok) throw new Error(data?.message || t('auth.forgotPassword.genericError'));
      setSent(true);
    } catch (err: any) {
    ...
    <main className="min-h-screen bg-white text-black flex items-center justify-center p-6">
      <div className="w-full max-w-sm space-y-6">
        ...
        {sent ? (
          ...
          <div className="space-y-3">
            <div className="flex flex-col gap-1">
              <label className="text-xs font-semibold text-gray-600">{t('auth.forgotPassword.email')}</label>
              <input
                type="email"
                value={email}
        ...
        <p className="text-center text-sm text-gray-500">
          <Link href="/login" className="underline hover:text-black">{t('auth.forgotPassword.backToSignIn')}</Link>
        </p>
      </div>
    </main>
  );
