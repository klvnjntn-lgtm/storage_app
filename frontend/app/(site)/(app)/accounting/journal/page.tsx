...
export default function JournalPage() {
  ...
  async function handleVoid(entry: JournalEntry) {
    const reason = prompt(`Void entry ${entry.entryNumber ?? entry.id}? Enter a reason:`);
    if (reason === null) return; // cancelled
    if (!reason.trim()) {
      setVoidError('A reason is required to void an entry.');
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        const body = await res.json().catch(() => null);
        setVoidError(body?.message ?? `Request failed (${res.status})`);
        return;
      }
      await loadEntries();
    } catch {
      setVoidError('Could not reach the server.');
    } finally {
      setVoidingId(null);
    }
  }
  ...
  async function handleSubmitManual(e: React.FormEvent) {
    ...
    if (manualLines.some((l) => !l.accountId)) {
      setManualError('Every line needs an account.');
      return;
    }
    if (manualLines.some((l) => !Number(l.amount) || Number(l.amount) <= 0)) {
      setManualError('Every line needs an amount greater than zero.');
      return;
    }
    if (!manualBalanced) {
      setManualError(`Not balanced — debits ${formatIDR(manualTotals.debit)} vs credits ${formatIDR(manualTotals.credit)}.`);
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        const body = await res.json().catch(() => null);
        setManualError(body?.message ?? `Request failed (${res.status})`);
        return;
      }
      setShowManual(false);
      setManualMemo('');
    ...
    >
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div className="min-w-0">
              <h1 className={`${display.className} text-xl sm:text-2xl font-bold tracking-tight truncate`}>Journal</h1>
              <p className="text-xs text-gray-500 truncate">Every entry ever posted to the ledger</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto p-4 sm:p-6">
        ...
        {showManual && (
          <form onSubmit={handleSubmitManual} className="border-2 border-blue-500/30 rounded-md bg-blue-50/40 p-4 mb-5 flex flex-col gap-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="flex flex-col gap-1">
                <label className="text-xs font-semibold text-gray-600">Memo (optional)</label>
            ...
            <div className="flex flex-col gap-2">
              <label className="text-xs font-semibold text-gray-600">Lines</label>
              ...
              >
                <Plus size={12} strokeWidth={2} /> Add line
              </button>
            </div>
            ...
            <div className={`flex items-center justify-between text-xs font-semibold rounded-md p-2 ${manualBalanced ? 'bg-green-50 text-green-700' : 'bg-amber-50 text-amber-700'}`}>
              <span>Debits: {formatIDR(manualTotals.debit)}</span>
              <span>Credits: {formatIDR(manualTotals.credit)}</span>
              <span>{manualBalanced ? 'Balanced' : 'Not balanced'}</span>
            </div>
            ...
            >
              ...
              {manualSaving ? 'Posting...' : 'Post entry'}
            </button>
          </form>
        )}
        ...
        {!loading && entries && entries.length === 0 && (
          <p className="text-sm text-gray-400 text-center py-8">No entries in this range.</p>
        )}
              ...
              <div key={entry.id} className="border-2 border-gray-300 rounded-md bg-white overflow-hidden">
                ...
                >
                  <div className="flex items-center gap-2.5 min-w-0">
                    ...
                    <span className="text-xs text-gray-400 shrink-0">{SOURCE_LABEL[entry.sourceType] ?? entry.sourceType}</span>
                ...
                {isExpanded && (
                  <div className="border-t border-gray-100">
                    ...
                    <div className="px-4 py-2.5 flex items-center justify-between">
                      {entry.reversalOfId && (
                        <span className="text-[11px] text-gray-400">Reversal of another entry</span>
                      )}
