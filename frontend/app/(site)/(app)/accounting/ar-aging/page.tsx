...
import { ArrowLeft, Wallet, Calendar, CheckCircle2, AlertTriangle, ChevronDown, ChevronUp } from 'lucide-react';
...
const BUCKETS: { key: keyof ARAgingReport['totals']; label: string; color: string }[] = [
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div className="min-w-0">
              <h1 className={`${display.className} text-xl sm:text-2xl font-bold tracking-tight truncate`}>AR Aging</h1>
              <p className="text-xs text-gray-500 truncate">Who owes you money, and how overdue it is</p>
            </div>
          </div>
        </div>
      </div>
        ...
        {!loading && report && (
          <>
            ...
            >
              ...
              <span>
                {report.reconciliation.matches
                  ? `Reconciled — matches the ledger's AR balance of ${formatIDR(report.reconciliation.arLedgerBalance)}.`
                  : `Doesn't reconcile: ledger AR balance ${formatIDR(report.reconciliation.arLedgerBalance)} vs sum of outstanding invoices ${formatIDR(report.reconciliation.sumOfOutstandingInvoices)}. Some invoice's paid status may not match what was actually posted.`}
              </span>
            </div>
            ...
            {report.byCustomer.length === 0 ? (
              <p className="text-sm text-gray-400 text-center py-8">No outstanding invoices as of this date.</p>
            ) : (
                ...
                {report.byCustomer.map((c) => {
                  ...
                  return (
                    <div key={key} className="border-2 border-gray-300 rounded-md bg-white overflow-hidden">
                      ...
                      >
                        <span className="text-sm font-semibold truncate">{c.customerName ?? 'No customer on file'}</span>
                            ...
                            <div key={line.invoiceId} className="px-3 py-2 flex items-center justify-between gap-3 text-xs border-b border-gray-50 last:border-b-0">
                              <div className="min-w-0">
                                ...
                                <span className="text-gray-400 ml-2">
                                  {line.daysOverdue > 0 ? `${line.daysOverdue}d overdue` : 'not yet due'}
                                </span>
                              </div>
