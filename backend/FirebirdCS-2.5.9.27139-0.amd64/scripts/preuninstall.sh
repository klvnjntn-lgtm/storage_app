...
if [ -f /opt/firebird/$SecurityDatabase ]; then 
	cp /opt/firebird/$SecurityDatabase /tmp 
...
for i in /opt/firebird/fb_guard /opt/firebird/firebird.log /opt/firebird/SYSDBA.password; do
	if [ -f $i ]; then
		rm -f $i
	fi
done

rm -f /opt/firebird/bin/fbmgr
